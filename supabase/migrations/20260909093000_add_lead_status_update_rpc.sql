...
-- Commercial lifecycle updates remain transactional: status, updated_at trigger
-- and history event either all succeed or all rollback together.
...
begin
  ...
  if (p_new_status <> 'lost' and p_lost_reason is not null)
    or (p_new_status = 'lost' and p_lost_reason is not null and p_lost_reason <> all(v_lost_reasons)) then
    raise exception 'INVALID_LOST_REASON' using errcode = '22023';
  end if;

  if not (
    (v_previous_status = 'new' and p_new_status in ('to_contact', 'contacted', 'qualified', 'appointment', 'proposal', 'won', 'lost'))
    or (v_previous_status = 'to_contact' and p_new_status in ('contacted', 'qualified', 'appointment', 'proposal', 'won', 'lost'))
    or (v_previous_status = 'contacted' and p_new_status in ('qualified', 'appointment', 'proposal', 'won', 'lost'))
    or (v_previous_status = 'qualified' and p_new_status in ('appointment', 'proposal', 'won', 'lost'))
    or (v_previous_status = 'appointment' and p_new_status in ('proposal', 'won', 'lost'))
