...
export function Footer() {
  ...
  return <footer className="footer"><Container>
    ...
    <div className="footer-sitemap"><FooterList title="Expertises" items={[["Toutes les solutions", "/solutions"], ["Financement & CEE", "/financement-cee"], ["Ressources", "/ressources"]]} /><FooterList title="Secteurs" items={sectors.map((item) => [item.title, `/secteurs/${item.slug}`])} /><FooterList title="Entreprise" items={[["À propos", "/a-propos"], ["Réalisations", "/realisations"], ["FAQ", "/faq"], ["Contact", "/contact"], ["Plan du site", "/plan-du-site"]]} /><div className="footer-contact"><h3>Nous contacter</h3><address><span>{contact.address}</span><a href={phoneHref} onClick={() => trackEvent('phone_clicked', { channel: 'footer' })}>{contact.phone}</a><a href={`mailto:${contact.email}`} onClick={() => trackEvent('email_clicked', { channel: 'footer' })}>{contact.email}</a><span>{contact.hours}</span><a href={contact.googleBusinessUrl} target="_blank" rel="noopener noreferrer">Nos avis Google</a></address></div></div>
    <div className="footer-bottom"><span>© {new Date().getFullYear()} {siteConfig.name}</span><div><Link to="/mentions-legales">Mentions légales</Link><Link to="/politique-de-confidentialite">Politique de confidentialité</Link><Link to="/gestion-des-cookies">Politique de cookies</Link><Link to="/conditions-generales-utilisation">CGU</Link></div></div>
  </Container></footer>;
}
