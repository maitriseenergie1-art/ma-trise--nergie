...
export function PageHero({eyebrow,title,text,image=heroImages.architecture,imageAlt='',breadcrumb,icon,children}) { return <section className="page-hero immersive-hero"><img className="hero-background" src={image} alt={imageAlt} fetchPriority="high"/><Container><div className="page-hero-copy">{breadcrumb&&<Breadcrumb current={breadcrumb}/>}<div className="hero-business-label">{icon&&<BusinessIcon name={icon} size={34}/>}<Eyebrow>{eyebrow}</Eyebrow></div><h1>{title}</h1><p className="lead">{text}</p>{children || <Button to="/eligibilite" sourceCta="page_hero_eligibility">Vérifier mon éligibilité</Button>}</div></Container></section>; }
