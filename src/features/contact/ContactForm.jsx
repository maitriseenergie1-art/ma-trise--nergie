...
export function ContactForm(){
  ...
  const validate = (values) => {
    ...
    if (!values.email || values.email.length > 254 || !/^\S+@\S+\.\S+$/.test(values.email)) errors.email = 'Renseignez un email valide.';
    if (values.company.length > 180) errors.company = 'Le nom de l’entreprise est trop long.';
    if (values.phone.length > 64) errors.phone = 'Le numéro de téléphone est trop long.';
    if (!values.need) errors.need = 'Choisissez un type de besoin.';
    if (values.message.length > 4000) errors.message = 'Le message est trop long.';
  ...
  return <form ref={formRef} onSubmit={submit} noValidate><h2>Être rappelé</h2><div className="form-grid"><label htmlFor="contact-first-name">Prénom<input id="contact-first-name" name="firstName" maxLength="100" autoComplete="given-name" aria-invalid={Boolean(fieldErrors.firstName)} aria-describedby={describedBy('firstName')}/>{fieldErrors.firstName&&<small id="firstName-error" className="field-error">{fieldErrors.firstName}</small>}</label><label htmlFor="contact-last-name">Nom<input id="contact-last-name" name="lastName" maxLength="100" autoComplete="family-name" aria-invalid={Boolean(fieldErrors.lastName)} aria-describedby={describedBy('lastName')}/>{fieldErrors.lastName&&<small id="lastName-error" className="field-error">{fieldErrors.lastName}</small>}</label><label htmlFor="contact-company">Entreprise<input id="contact-company" name="company" maxLength="180" autoComplete="organization" aria-invalid={Boolean(fieldErrors.company)} aria-describedby={describedBy('company')}/>{fieldErrors.company&&<small id="company-error" className="field-error">{fieldErrors.company}</small>}</label><label htmlFor="contact-phone">Téléphone<input id="contact-phone" name="phone" type="tel" maxLength="64" autoComplete="tel" aria-invalid={Boolean(fieldErrors.phone)} aria-describedby={describedBy('phone')}/>{fieldErrors.phone&&<small id="phone-error" className="field-error">{fieldErrors.phone}</small>}</label><label className="full" htmlFor="contact-email">Email<input id="contact-email" name="email" type="email" maxLength="254" autoComplete="email" aria-invalid={Boolean(fieldErrors.email)} aria-describedby={describedBy('email')}/>{fieldErrors.email&&<small id="email-error" className="field-error">{fieldErrors.email}</small>}</label><label className="full" htmlFor="contact-need">Type de besoin<select id="contact-need" name="need" defaultValue="" aria-invalid={Boolean(fieldErrors.need)} aria-describedby={describedBy('need')}><option value="">Choisir un sujet</option><option value="etude">Étude énergétique</option><option value="travaux">Travaux</option><option value="pilotage">Pilotage</option><option value="financement">Financement</option></select>{fieldErrors.need&&<small id="need-error" className="field-error">{fieldErrors.need}</small>}</label><label className="full" htmlFor="contact-message">Message (optionnel)<textarea id="contact-message" name="message" rows="4" maxLength="4000" aria-invalid={Boolean(fieldErrors.message)} aria-describedby={describedBy('message')}/>{fieldErrors.message&&<small id="message-error" className="field-error">{fieldErrors.message}</small>}</label></div><div className="contact-trap" aria-hidden="true"><label htmlFor="contact-website">Votre site web<input id="contact-website" name="website" type="url" tabIndex="-1" autoComplete="off"/></label></div><label className="privacy"><input name="privacy" type="checkbox" aria-invalid={Boolean(fieldErrors.privacy)} aria-describedby={describedBy('privacy')}/> J’ai lu l’information relative à l’utilisation de mes données dans la <Link to="/politique-de-confidentialite">politique de confidentialité</Link>.</label>{fieldErrors.privacy&&<small id="privacy-error" className="field-error">{fieldErrors.privacy}</small>}<div aria-live="assertive">{error&&<p className="error" role="alert">{error}</p>}</div><Button type="submit" sourceCta="contact_submit" disabled={status==='submitting'} aria-disabled={status==='submitting'}>{status==='submitting'?'Envoi en cours…':'Être rappelé'}</Button><p className="calendly">Ou choisir directement un rendez-vous<br/><span>Emplacement réservé à une future intégration Calendly.</span></p></form>;
}
